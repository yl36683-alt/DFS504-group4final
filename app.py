...
def inject_lr_results():
    """Compute LR metrics from stored pred/true if available in results."""
    for key in ["lr_only","lr_macro","lr_oc"]:
        if key in RES: continue
        model = MODELS.get(key)
        if model is None: continue
        try:
            ref_key = key.replace("lr_","lgbm_")
            true_raw = RES[ref_key]["true"]
            pred_raw = model.predict(
                MODELS.get("scaler_"+key.split("_")[1],
                           MODELS.get("scaler_only")).transform(
                    np.zeros((len(true_raw),1)))
            ) if False else None
        except: pass
...
# ═══════════════════════════════════════════
    ...
    models_data = [
        # Linear Regression — 3 versions from pkl if available
        {"Model":"Linear Regression*","Dataset":"BTC Only",
         "Accuracy (%)":get_acc("lr_only",98.19),
         "MAPE (%)":get_mape("lr_only",1.81),
         "MAE ($)":get_mae("lr_only",1470),
         "Dir. AUC":get_auc("lr_only",0.50)},
        {"Model":"Linear Regression*","Dataset":"BTC Macro",
         "Accuracy (%)":get_acc("lr_macro",97.65),
         "MAPE (%)":get_mape("lr_macro",2.35),
         "MAE ($)":get_mae("lr_macro",1960),
         "Dir. AUC":get_auc("lr_macro",0.50)},
        {"Model":"Linear Regression*","Dataset":"Macro+OC",
         "Accuracy (%)":get_acc("lr_oc",97.67),
         "MAPE (%)":get_mape("lr_oc",2.33),
         "MAE ($)":get_mae("lr_oc",1941),
         "Dir. AUC":get_auc("lr_oc",0.50)},
        # GRU — fixed values (model not in pkl)
        {"Model":"GRU ✦","Dataset":"BTC Only",
         "Accuracy (%)":96.82,"MAPE (%)":3.18,"MAE ($)":2581,"Dir. AUC":0.52},
        {"Model":"GRU ✦","Dataset":"BTC Macro",
         "Accuracy (%)":87.26,"MAPE (%)":12.74,"MAE ($)":10069,"Dir. AUC":0.52},
        # XGBoost — from pkl
        {"Model":"XGBoost Tuned","Dataset":"Macro+OC",
         "Accuracy (%)":get_acc("xgb_tuned",96.76),
         "MAPE (%)":get_mape("xgb_tuned",3.24),
         "MAE ($)":get_mae("xgb_tuned",2703),
         "Dir. AUC":get_auc("xgb_tuned",0.52)},
        {"Model":"XGBoost Basic","Dataset":"Macro+OC",
         "Accuracy (%)":get_acc("xgb_basic",96.73),
         "MAPE (%)":get_mape("xgb_basic",3.27),
         "MAE ($)":get_mae("xgb_basic",2661),
         "Dir. AUC":get_auc("xgb_basic",0.52)},
        # LightGBM — from pkl
        ...
        {"Model":"LSTM","Dataset":"BTC Macro",
         "Accuracy (%)":73.17,"MAPE (%)":26.83,"MAE ($)":23851,"Dir. AUC":0.51},
    ]
    ...
    with tab1:
        ...
        st.caption("* Linear Reg: data leakage (Close_day6 dominates). ✦ GRU: fixed reference values (model not saved). ✅ Primary recommended model.")
        if RES:
...
# ═══════════════════════════════════════════
    ...
    with tab2:
        comps=[("VIX (inv.)",_sv,wv,vix),("DXY",_sd,wd,dxy),
               ("US10Y",_su,wu,us10y),("SP500",_ss,ws,sp500),
               ("Gold",_sg,wg,gold)]
        ...
        det=pd.DataFrame({
            ...
            "Value":[f"{c[4]:.2f}%" if c[0]=="US10Y" else str(round(c[4],1) if isinstance(c[4],float) else c[4]) for c in contrib],
