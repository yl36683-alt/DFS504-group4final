        ...
        if sel:
            ...
            for i, ind in enumerate(sel, 1):
                fig.add_trace(go.Scatter(x=DF["Date"], y=DF[ind], mode="lines",
                    ...
                    fill="tozeroy",
                    fillcolor=f"rgba{tuple(list(int(cmap.get(ind,BLUE).lstrip('#')[j:j+2],16) for j in (0,2,4))+[0.05])}"),
                    row=i, col=1)
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    tab1, tab2, tab3, tab4 = st.tabs(["Full Comparison","Accuracy Charts","Cross-Validation","Hyperparameters"])
    ...
    with tab1:
        def rc(row):
            if "✅" in str(row["Model"]): return ["background-color:#0a2018"]*len(row)
            if "*"  in str(row["Model"]): return ["background-color:#1a1500"]*len(row)
            return [""]*len(row)
...
# ══════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3 = st.tabs(["Feature Importance","Macro Direction Signals","Interpretation Guide"])
